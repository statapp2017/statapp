...
#' Delete all the numbers, spaces, punctuations, and special characters present in the corpus.
#' 
#' @param corpus A collection of documents containing text (obtained with Corpus).
#' @return A new corpus without numbers, spaces, punctuations, and special characters.
...
#' of stopwords, punctuations etc...). Preprocessing of the database.
#' 
...
preprocess_text <- function(data, column, sparseness = 0.99) {
  data$corrige <- sapply(X = data[, column], FUN = spell_checker) # Spell checking
  dico <- creation_dico(data$corrige)
  dataframe_corrige <- lemmatizer_dataframe(data, dico, column) # Lemmatisation
