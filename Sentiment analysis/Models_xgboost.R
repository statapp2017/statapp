...
models_xgboost<-function(params,dtm_ep,data,column,number_class,number_models,train_test_split=.8,train_valid_split=.8){
  ...
  breaking<-unique(quantile(data_tot$categorie, probs = seq(0, 1, by = round(1/number_class,digits=2))))
  data_tot$categorie<-cut(data_tot$categorie,breaks=breaking,labels=as.factor(0:(length(breaking)-2)))
  ...
  list(models=mod$models,dvalid=dvalid,place=mod$entrainement,notes=data_tot[,1],train_test=train_test,nom_colonne=colnames(dtm_ep))
}